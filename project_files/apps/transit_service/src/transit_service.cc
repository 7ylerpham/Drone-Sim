...
public:
    ...
    void ReceiveCommand(const std::string& cmd, JsonObject& data, JsonObject& returnValue) {
        ...
        if (cmd == "CreateEntity") {
            ...
            //  get instance of weather singleton and update weather every 10 seconds
            Weather* weather = Weather::GetWeather();
            if ((int)time != lastUpdate && (int)time % 10 == 0){
                std::cout<<"THE WEATHER WILL BE RANDOMIZED AT TIME "<<time<<std::endl;
    ...
    void SendWeatherDetails() {
        JsonObject weatherDetails;
        Weather* weather = Weather::GetWeather();
        weatherDetails["weatherType"] = weather->GetWeatherType();
        weatherDetails["windSpeed"] = weather->GetWindSpeed();
        weatherDetails["windDirection"] = weather->GetWindDirection();
        SendEventToView("SendWeatherDetails", weatherDetails);
    }
...
private:
    ...
    // keeps track of last weather update time to prevent multiple weather randomizations
    int lastUpdate = -1;
};
